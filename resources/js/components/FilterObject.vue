    ...
    <div class="filtersTabs-box">
        <form @submit.prevent="filterData()" method="POST" :model="form">
            <ul class="filtersTabs-ul">
                ...
                <li class="filtersTabs-ul-li">
                    <button type="button" @click="active = 2, form.object_deals = 'daily'" class="filtersTabs-ul-li-btn"
                        :class="{ 'filtersTabs-ul-li-btn-active': active == 2 }">Посуточно</button>
                </li>
                <li class="filtersTabs-ul-li">
                    <button type="button" @click="active = 3, form.object_deals = 'new_building'" class="filtersTabs-ul-li-btn"
                        :class="{ 'filtersTabs-ul-li-btn-active': active == 3 }">Новостройка</button>
                </li>
            </ul>
            <div class="filters-view">
                <div class="filters-view-wrapper">
                    <div v-if="active != 3" class="filtr_block-item filter-object-type item-width-1">
                        <Dropdown @click="open = null" v-model="form.object_type_id" optionValue="id"
                            :options="objectTypes" optionLabel="name_ru" placeholder="Выбирать"
                            panelClass="p-multiselect-panell" />
                    </div>
                    <div v-if="active == 3" class="filtr_block-item filter-object-type item-width-1">
                        <Dropdown @click="open = null" v-model="form.object_new" optionValue="id"
                            :options="object_time_type" optionLabel="name_ru" placeholder="Выбирать"
                            panelClass="p-multiselect-panell" />
                    </div>
                    <div v-if="form.object_type_id == 3 && active != 3" class="filtr_block-item item-width-2">
                        <MultiSelect v-model="form.object_types_property_id" @click="open = null"
                            :options="objectProperty" optionLabel="name_ru" optionValue="id" display="chip"
                            placeholder="Выбирать" :filter="true" panelClass="p-multiselect-panell" />
                    </div>
                    <div v-if="form.object_type_id != 3" class="filtr_block-item item-width-2">
                        <button type="button" @click="toggle(2)" class="filtr_block-item-btn"
                            :class="{ 'filtr_block-item-btn-active': 2 == open }">{{ form.room_count.length > 0 ? text :
                                    'Комнат'
                            }}</button>
                        <div class="filtr_block-item-dropdown" v-if="2 === open">
                            <ul class="room_count-list">
                                <li v-for="item in room_count_val" :key="item.id" class="room_count-list_li">
                                    <label :for="`room_count_${item.id}`" class="single_button_select_box_label"
                                        :class="{ 'active': item.isActive }">
                                        <Checkbox :id="`room_count_${item.id}`" name="room_count[]" :value="item.id"
                                            v-model="form.room_count" class="single_button_select_box_label_inpt d-none"
                                            @click="isChecked(item)" @change="getName(item.id)" />
                                        <span class="single_button_select_box_label_span"
                                            :class="{ 'active_span': item.isActive }">{{ item.id }}</span>
                                    </label>
                                </li>
                            </ul>
                        </div>
                    ...
                    <div class="filtr_block-item item-width-2">
                        <button type="button" @click="toggle(3)" class="filtr_block-item-btn"
                            :class="{ 'filtr_block-item-btn-active': 3 == open }">
                            {{ (form.from_price != '' && form.to_price != '') ? (`${vueNumberFormat(form.from_price, {})} - ${vueNumberFormat(form.to_price, {})}`) : form.from_price != '' ? `от ${vueNumberFormat(form.from_price, {})}` : form.to_price != '' ? `до ${vueNumberFormat(form.to_price, {})}` : 'Цена'
                            }}
                            </button>
                            ...
                            <div class="filtr_block-item-dropdown-wrapper">
                                <div class="filtr_block-item-dropdown-wrapper-item">
                                    <div class="filtr_block-item-dropdown-wrapper-item-children">
                                        <input placeholder="от" type="number" class="_025a50318d--input--cptMO"
                                            v-model="form.from_price" />
                                    </div>
                                </div>
                                <div class="filtr_block-item-dropdown-wrapper-item">
                                    <div class="filtr_block-item-dropdown-wrapper-item-children">
                                        <input placeholder="до" type="number" class="_025a50318d--input--cptMO"
                    ...
                    <div class="filtr_block-item item-width-2">
                        <Dropdown @click="open = null" v-model="form.region_id" @change="getDistricts()"
                    ...
                    <div class="filtr_block-item" style="flex: 1 0;">
                        <MultiSelect v-model="form.quarter_id" :options="quarters" optionLabel="name_ru"
                            optionValue="id" display="chip" placeholder="Выбирать улица" :filter="true"
                            panelClass="p-multiselect-panell" />
                    </div>
                </div>
            </div>
